...
**読み分け:** **ルール・定義・学習論点・完了済みチェックリスト**は本ファイル。**進捗の数値・全フェーズのタスク表（時間・実動・備考）・クイズ単独ログ**は [`dev_docs/inception_progress_snapshot.md`](inception_progress_snapshot.md)（ダッシュボード。**常に最新でなくてよい**。セッション終了時またはドライバー指示で更新。**計画の基準日は本ファイル冒頭の「更新日」を正とし**、スナップショットの記録日とずれていても差異は度外視してよい）。
...
- ファイルを直接編集・作成しない（運用ドキュメント・セッションログ・`quizzes/` 配下のクイズ MD は除く）
...
- **クイズ MD の作成**（`quizzes/` 配下。問題文・選択肢・「自分の回答」・一次資料のみを先に書くフローは下記「クイズ運用ルール」参照）。新規作成時は **作成計画h** を見積もり、[`inception_progress_snapshot.md`](inception_progress_snapshot.md) **B3** に **作成計画h** 列で記入できる
- 一次資料のURL提示
...
**基本: 1タスク = 1セッション**（[`inception_progress_snapshot.md`](inception_progress_snapshot.md) B4 のタスク行 1 行に相当）
...
### 実作業時間の算出（0.5 時間単位で切り上げ）

セッションログの「実作業時間」は、ドライバーが記録した**開始・終了時刻**（`date '+%Y-%m-%d %H:%M'`）から分数を求め、**0.5 時間単位で切り上げ**る（例: 57 分 → **1.0h**、31 分 → **1.0h**、30 分 → **0.5h**）。休憩・中断を除く区間を表に分けて合算してもよい。以降のセッションログでも同じルールを適用する。

### タスク表の「実動」列（セッションログとの対応）

[`inception_progress_snapshot.md`](inception_progress_snapshot.md) B4 の各タスク行の **「実動」** は、そのタスクに対応する **セッションログに記載した実作業時間**（上記 0.5h 切り上げ後）を転記する。単位は **h**（例: `2.0h`、`5.0h`）。未着手・未記録は **`-`**。

**1 セッションで隣接タスクのみをまとめた場合（例: タスク n-m と n-m+1）** で、**合計の実作業時間しかログに無い**ときは、**番号が若い行（n-m）に合計を書き**、**続く行（n-m+1）は `-`** とする（内訳が後から分かったら行ごとに修正してよい）。

**1 セッションに事前クイズとタスクが混在**するなど、タスク表の行と 1:1 で対応しない場合は、該当行は `-` とし、**備考にセッションログ番号**を書く。

セッション終了時（タスク完了のたび）、ドライバー確認のうえ **本列を更新**する（主にスナップショット B4）。AI がセッションログ作成時に更新してよいのは、ドライバーがその内容を承認した場合に限る（既存の「phase_plan.md の更新手順」に準拠）。

### クイズ単独セッションの実作業時間（一覧表）

**→ [`inception_progress_snapshot.md`](inception_progress_snapshot.md)** に移動した（表・集計・運用はそちらを編集する）。
...
- dev_docs/inception_progress_snapshot.md（進捗数値・**全フェーズのタスク表**・クイズ単独ログ。任意）
...
1. **ドライバーの確認のうえ**、完了したタスクを `phase_plan.md` の「完了済み」に追記（中間進捗のみのセッションではセッションログに記録し、完了チェックは保留可。手順は上記「phase_plan.md の更新手順」参照）。あわせて、[`inception_progress_snapshot.md`](inception_progress_snapshot.md) B4 の該当タスク行の **「実動」** にセッションログの実作業時間を転記する（`phase_plan` にタスク表を二重に持たない運用。運用は上記「タスク表の『実動』列」参照）。
2. **クイズ単独セッション**の場合は、[`inception_progress_snapshot.md`](inception_progress_snapshot.md) の **B3** に **1 行追加**し、**実動** を記入する（タスク完了チェックは不要なことが多い）。
3. **進捗・クイズ単独**（[`inception_progress_snapshot.md`](inception_progress_snapshot.md)）の **更新日**・表を、セッションログと突き合わせて再計算・更新する（ドライバー確認のうえ）。**さらにファイルを分割するか**はドライバーに確認すること。
4. 次のセッションログを作成してコミット:
...
> 実作業時間: X.Xh（ユーザー自己申告）
> 計画時間: Xh（`inception_progress_snapshot.md` B4 の該当タスクから転記）
...
4. git commit（**提案のみ。実行はユーザーの明示的な指示があるまで行わない**）:
```bash
git add .
...
**定義・全体サマリ・フェーズ別表・クイズ単独ログ**は **[`inception_progress_snapshot.md`](inception_progress_snapshot.md)** に切り出した（数値の一次情報はセッションログ。再計算の手順は同ファイルの「運用」参照）。
...
| 4 | Docker Compose 統合 + secrets | 14h | pre + post（healthcheck は課題非明示のため計画から除外・タスク 4-1 判断）|
...
**タスク一覧（時間・実動・実施日・備考）・BAC／見込み:** [`inception_progress_snapshot.md`](inception_progress_snapshot.md)（冒頭の **A'** 進行中抜粋・**B1/B2** 数値・**B4** フェーズ 0 タスク表）。
...
**タスク一覧（時間・実動・備考）・BAC／見込み:** [`inception_progress_snapshot.md`・B4・フェーズ 1](inception_progress_snapshot.md#b4-フェーズ別タスク詳細全フェーズ)
...
**タスク一覧（時間・実動・備考）・BAC／見込み:** [`inception_progress_snapshot.md`・B4・フェーズ 2](inception_progress_snapshot.md#b4-フェーズ別タスク詳細全フェーズ)
...
**タスク一覧（時間・実動・備考）・BAC／見込み:** [`inception_progress_snapshot.md`・B4・フェーズ 3](inception_progress_snapshot.md#b4-フェーズ別タスク詳細全フェーズ)
...
**メモ（2026-04-12 タスク 4-1）:** 課題書に **healthcheck の明示なし**。`depends_on` の起動順と **entrypoint 側の待機**で足りる方針。提出物の `docker-compose.yml` に **healthcheck は含めない**。一次資料の healthcheck 範囲は**読了のみ**。
...
- `healthcheck` の構文と `depends_on: condition:`（**理解用**。提出物には healthcheck を書かない方針）
- Docker network（bridge）の仕組み
- `restart: always` の動作

**タスク一覧（時間・実動・備考）・BAC／見込み:** [`inception_progress_snapshot.md`・B4・フェーズ 4](inception_progress_snapshot.md#b4-フェーズ別タスク詳細全フェーズ)（**進行中の抜粋**は同ファイル冒頭 **A'**）
...
- `depends_on` の保証範囲（起動順）と、アプリ側待機が必要な理由（healthcheck を使わない前提でも説明できること）
...
**タスク一覧（時間・実動・備考）・BAC／見込み:** [`inception_progress_snapshot.md`・B4・フェーズ 5](inception_progress_snapshot.md#b4-フェーズ別タスク詳細全フェーズ)
...
**タスク一覧（時間・実動・備考）・BAC／見込み:** [`inception_progress_snapshot.md`・B4・フェーズ 6](inception_progress_snapshot.md#b4-フェーズ別タスク詳細全フェーズ)
...
**タスク一覧（時間・実動・備考）・BAC／見込み:** [`inception_progress_snapshot.md`・B4・フェーズ 7](inception_progress_snapshot.md#b4-フェーズ別タスク詳細全フェーズ)
...
**タスク一覧（時間・実動・備考）・BAC／見込み:** [`inception_progress_snapshot.md`・B4・フェーズ 8](inception_progress_snapshot.md#b4-フェーズ別タスク詳細全フェーズ)

### 総合クイズ → `quizzes/0800_final_review_quiz_inception.md`
全フェーズ横断。以下のカテゴリを網羅:
- Docker 基礎（PID 1, ベースイメージ, Dockerfile ベストプラクティス）
- Docker Compose（networks, volumes, secrets, restart、healthcheck は任意理解）
...
| 起動順制御 | `depends_on` のみ | **`depends_on` + entrypoint 側の DB 待機**（healthcheck は課題非明示のため未採用） |
